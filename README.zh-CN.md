...
????? GitHub Action ??? `scripts/deploy/index.ts`??????????

- ????? D1 ???
- ????? KV Namespace
- ????? R2 ?? Bucket
- ?? `wrangler.json`?`wrangler.email.json`?`wrangler.cleanup.json`
- ???? `wrangler d1 migrations apply --remote`
- ?? Pages??? Worker ? Cleanup Worker
...
- `CUSTOM_DOMAIN`????
- ??????? `AUTH_GITHUB_ID`?`AUTH_GITHUB_SECRET`?`AUTH_GOOGLE_ID`?`AUTH_GOOGLE_SECRET`?`AUTH_SECRET`

### ???????

- ????? `DATABASE_ID`??????? `DATABASE_NAME` ??????? D1
- ????? `KV_NAMESPACE_ID`??????? `KV_NAMESPACE_NAME` ??????? KV
- ????? `R2_ATTACHMENTS_BUCKET`????????????????? R2 Bucket
